...
// Variante C: calculadora do vazamento adaptada ao nicho (EXP-027).
const CALC_ITENS = [
  { f: "matéria", t: "O pó moído há meses", s: "o aroma foi embora antes de você abrir o pacote" },
  { f: "matéria", t: "A água da torneira direto", s: "cloro e dureza mudando o sabor antes do pó" },
  { f: "superfície", t: "A moagem única pra tudo", s: "o mesmo pó pro coador e pra prensa, sempre errado num deles" },
  { f: "superfície", t: "O filtro sem escaldar", s: "gosto de papel na primeira xícara do dia" },
  { f: "contato", t: "A água fervendo no pó", s: "temperatura de queimar, amargor de graça" },
  { f: "contato", t: "O despejo de uma vez só", s: "sem bloom, sem espera, extração pela metade" },
  { f: "constância", t: "A dose no olho", s: "cada dia uma xícara diferente, nenhuma repetível" },
  { f: "constância", t: "O tempo de coada sem relógio", s: "o acerto de ontem impossível de repetir hoje" },
];
const CALC_INIT = [true, true, true, false, true, true, true, false];
...
export default function EbookPremium() {
  ...
  const [calc, setCalc] = useState<boolean[]>(CALC_INIT);
  const total = calc.filter(Boolean).length;
  ...
  return (
    <>
      ...
      {/* Variante C: a calculadora do vazamento como cabeçalho (c5, HC 24/07) */}
      <header className="abre">
        <span className="kicker" style={{ display: "block", marginBottom: 16 }}>O balcão mora nas variáveis</span>
        <h1 className="calc-h1">Quantas variáveis jogam contra a sua xícara?</h1>
        <p className="abre-sub">Marque o que descreve o seu café de casa hoje. Cada variável tem ajuste de minutos no guia.</p>
      </header>
      <section className="calc">
        <div className="maquina">
          <div className="painel">
            <p className="rot">variáveis contra a sua xícara</p>
            <p className="tot">{total} <span className="de8">de 8</span></p>
            <p className="ano">cada uma separa o seu coador do balcão</p>
          </div>
          <div className="itens">
            {CALC_ITENS.map((it, i) => (
              <label key={it.t} className={"item" + (calc[i] ? " on" : "")}>
                <input
                  type="checkbox"
                  checked={calc[i]}
                  onChange={() => setCalc((c) => c.map((v, k) => (k === i ? !v : v)))}
                />
                <span className="caixa-ck" aria-hidden="true" />
                <span className="it-t"><b>{it.t}</b><span>{it.s}</span></span>
                <span className="it-v">{it.f}</span>
              </label>
            ))}
          </div>
        ...
        <p className="obs">As variáveis vêm da ficha da coada: matéria, superfície, contato e constância. O checklist completo está no guia.</p>
      </section>
...
.kicker{font-family:var(--mono);font-size:11px;font-weight:500;letter-spacing:.24em;text-transform:uppercase;color:var(--bright)}
        ...
        /* ===== Variante C (EXP-027): calculadora + FAQ ===== */
/* Variante C: calculadora do vazamento (cabeçalho) */
        .abre{max-width:680px;margin:0 auto;padding:clamp(2.6rem,7vh,4.4rem) 1.4rem 1.6rem;text-align:center}
        .calc-h1{font-family:var(--serif);font-weight:900;font-size:clamp(1.9rem,5.4vw,3rem);line-height:1.14;color:#fff;letter-spacing:-.01em;text-wrap:balance}
        .abre-sub{font-size:clamp(15.5px,2vw,17px);color:var(--text);margin-top:14px;max-width:48ch;margin-inline:auto;line-height:1.6}
        .calc{max-width:760px;margin:0 auto;padding:18px 16px 4.4rem}
        .maquina{background:var(--bg-deep);border:1px solid var(--hair);border-radius:14px;overflow:hidden}
        .painel{padding:26px 22px;text-align:center;border-bottom:1px solid var(--hair)}
        .painel .rot{font-family:var(--mono);font-size:11px;letter-spacing:.24em;text-transform:uppercase;color:var(--text-dim)}
        .painel .tot .de8{font-size:.34em;color:var(--text-dim);font-weight:400;letter-spacing:0}
        .painel .tot{font-family:var(--serif);font-weight:900;font-size:clamp(52px,10vw,84px);color:var(--bright);line-height:1.05;margin-top:8px;font-variant-numeric:tabular-nums;letter-spacing:-.02em}
        .painel .ano{font-size:13.5px;color:var(--text-dim);margin-top:4px}
        .itens{padding:6px 0}
        .item{display:flex;align-items:center;gap:14px;padding:13px 22px;cursor:pointer;border-bottom:1px solid var(--hair);user-select:none}
        .item:last-child{border-bottom:0}
        .item input{position:absolute;opacity:0;pointer-events:none}
        .caixa-ck{flex:0 0 22px;height:22px;border:1.5px solid var(--text-dim);border-radius:5px;display:flex;align-items:center;justify-content:center;transition:all .18s}
        .caixa-ck::after{content:"✓";font-size:13px;font-weight:700;color:#140B04;opacity:0;transform:scale(.4);transition:all .18s}
        .item.on .caixa-ck{background:var(--bright);border-color:var(--bright)}
        .item.on .caixa-ck::after{opacity:1;transform:none}
        .it-t{flex:1;min-width:0}
        .it-t b{display:block;font-size:15px;color:var(--text);font-weight:600}
        .item.on .it-t b{color:#fff}
        .it-t span{font-size:12.5px;color:var(--text-dim)}
        .it-v{font-family:var(--mono);font-size:14px;color:var(--text-dim);white-space:nowrap;font-variant-numeric:tabular-nums}
        .item.on .it-v{color:var(--bright);font-weight:700}
        .rodape-calc{padding:22px;text-align:center;border-top:1px solid var(--hair)}
        .rodape-calc .btn{padding:15px 32px;font-size:16.5px}
        @media(max-width:560px){.item .it-v{display:none}}
        .calc-micro{font-size:13.5px;color:var(--text-dim);margin-top:13px;font-variant-numeric:tabular-nums}
        .calc-micro s{opacity:.8}
        .calc-micro b{color:#fff;font-weight:600}
.obs{text-align:center;font-size:13px;color:var(--text-dim);margin-top:16px;max-width:560px;margin-inline:auto;line-height:1.5}
